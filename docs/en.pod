...
A USSD query is a "phone number" starting with "*1" and ending with 
...
This works with all GSM network providers that I know of (which,
admittedly, doesn't say much). Shows your prepaid account balance.
B<Default>, if no other USSD query is given on the command line.
Basically, USSD queries are B<not> portable between networks, but this
one seems to be a convention every network adheres to. 
...
Replenish your prepaid account with a "Cashcode". Please note that a
cash code consists only of numbers! Dashes in your cash code are simply
used for readability, don't enter them. Use only the numbers.
